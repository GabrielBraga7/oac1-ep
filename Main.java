...
public class Main{
    public static void main(String[] args){
        ...
        if(n <= 0 || x <= 0 || x == 1){
            ...
            int p = calculaP(x, n);    //executa a funcao calcula p
            System.out.println(p == Integer.MAX_VALUE ? "P = Infinito" : "P = " + p);
        }
        sc.close();
    }
    ...
    public static int calculaP(double x, double n){
        if(x > n){
            ...
            if(x < 1)
                return Integer.MAX_VALUE;   //P aumenta mas x^P diminui, ou seja, x^P sempre será menor do que N (ex: x = 1/4 e N = 1/2)
            else
